...
class CustomAccountManager(BaseUserManager):
    ...
    def create_user(self, email, user_name, name, password, **other_fields):
        ...
        if other_fields.get('is_teacher') is True:
...
class empAssignTime(models.Model):
    period = models.CharField(max_length=50, choices=empTime_slots)
    day = models.CharField(max_length=15, choices=DAYS_OF_WEEK)
...
class empAttendance(models.Model):
    ...
    empAssignTime = models.ForeignKey(empAssignTime, on_delete=models.CASCADE)
    date = models.DateField()
    status = models.BooleanField()

    def __str__(self):
        eName = Employee.objects.get(name=self.student)
        return eName
